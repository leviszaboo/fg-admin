...
export default function Post({ id, urls, title, subTitle }: PostProps) {
  ...
  return (
    <>
      <div
        className={`border-2 border-brown rounded-lg h-80 flex flex-column overflow-hidden items-center content-center justify-center text-center`}
      >
        <div className="h-full w-full flex flex-column justify-center items-center image-radius-inner border-4 border-white overflow-hidden">
          <div className="relative h-full w-full bg-black">
            ...
            <div className="absolute bottom-0 left-0 pb-3 pl-4 z-50 text-left">
              <h3 className="text-shadow text-white text-md font-medium pb-0">
                {title}
              </h3>
              <h3 className="text-shadow text-white text-md font-medium">
                {subTitle}
              </h3>
            </div>
              ...
              {urls.map((url) => (
                ...
                >
                  <Circle
                    size={8}
                    fill="white"
                    className={`${urls.length > 1 ? "transparent" : null} text-white`}
                  />
                </div>
              ))}
            </div>
            <img
              className={`object-cover min-h-full -z-50 brightness-80`}
              src={url}
            />
          </div>
        </div>
